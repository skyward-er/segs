use egui::{Response, Sense, Ui, Vec2, Widget, vec2};
...
impl Separator {
    ...
    pub fn ui(self, ui: &mut Ui) -> Response {
        ...
        let is_horizontal_line = is_horizontal_line.unwrap_or_else(|| !ui.layout().main_dir().is_horizontal());
        ...
        if ui.is_rect_visible(response.rect) {
            ...
            stroke.color = ui.app_style().widgets.noninteractive.bg_stroke_color;
            let painter = ui.painter();
            if is_horizontal_line {
                painter.hline((rect.left() - grow)..=(rect.right() + grow), rect.center().y, stroke);
            } else {
                painter.vline(rect.center().x, (rect.top() - grow)..=(rect.bottom() + grow), stroke);
            }
        }
    ...
    pub fn ui_with_style(self, ui: &mut Ui, style_mod: impl FnOnce(&mut Style)) -> Response {
        ui.with_style_override(style_mod, |ui| self.ui(ui)).inner
    }
}
