use egui::{
    Align2, CursorIcon, Id, Key, Modifiers, NumExt as _, Rect, Response, Sense, Shape, Stroke, StrokeKind, TextStyle,
    Ui, Widget, WidgetInfo, emath, epaint, text, vec2,
};
...
/// ui.add(segs-ui::DragValue::new(&mut my_f32).speed(0.1));
/// # });
/// ```
...
impl<'a> DragValue<'a> {
    ...
    pub fn octal(self, min_width: usize, twos_complement: bool) -> Self {
        assert!(min_width > 0, "DragValue::octal: `min_width` must be greater than 0");
        if twos_complement {
...
impl Widget for DragValue<'_> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        // Hardcoded font size to match the default font size of TextEdit.
        let font_id = ui.app_style().base_font_of(13.);
        ...
        let mut response = if is_kb_editing {
            ...
            let drag_value_text = format!("{prefix_text}{value_text}{suffix_text}");
            let text_color = ui.visuals().widgets.inactive.text_color();

            let galley = ui
                .painter()
                .layout_no_wrap(drag_value_text.clone(), font_id.clone(), text_color.clone());

            let desired_width = galley.size().x + ui.spacing().interact_size.x;
            let desired_width = desired_width.at_least(galley.size().x);
            ...
            let hover_t = ui.ctx().animate_bool(id.with("_hover_t"), response.hovered());
            let active_t = ui.ctx().animate_bool(id.with("_active_t"), response.has_focus());

            let style = &ui.app_style().drag_value;
            let background_color = style
                .inactive_fill
                .lerp_to_gamma(style.hover_fill, hover_t)
                .lerp_to_gamma(style.active_fill, active_t);

            let frame_rect = rect.expand(0.5);
            let shape = epaint::RectShape::new(frame_rect, 3, background_color, Stroke::NONE, StrokeKind::Inside);
            ...
            if response.clicked() {
                ...
                if delta_value != 0.0 {
                    ...
                    let rounded_new_value =
                        emath::smart_aim::best_in_range_f64(precise_value - aim_delta, precise_value + aim_delta);
