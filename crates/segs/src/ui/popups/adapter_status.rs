...
impl<'a> ConnectionPopup<'a> {
    ...
    /// Render the connection popup contents directly into an existing UI.
    /// Useful for embedding inside another container (e.g. a modal).
    pub fn render_contents(_self: Self, ui: &mut Ui) {
        connection_ui(ui);
    }

    /// Render the connection popup contents into an existing UI without needing
    /// a `ConnectionPopup` instance. Useful to embed inside other containers.
    pub fn render_contents_ui(ui: &mut Ui) {
        connection_ui(ui);
    }
}
