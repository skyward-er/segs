...
use crate::{
    error::ErrInstrument,
    get_message_broker,
    mavlink::{extract_from_message, MessageData, ROCKET_FLIGHT_TM_DATA},
    message_broker::MessageSubscription,
...
pub struct Plot2DPane {
    ...
    #[serde(skip)]
    sub: Option<MessageSubscription>,
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _: TileId) -> PaneResponse {
        ...
        if self.plot_active {
            if let Some(sub) = &self.sub {
                ...
                // TODO: should we handle messages immediately at subscription instead of delaying?
                self.sub = Some(get_message_broker!().subscribe(ROCKET_FLIGHT_TM_DATA::ID));
            }
