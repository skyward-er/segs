...
impl App {
    ...
    fn process_incoming_messages(&mut self) {
        ...
        if !self.message_bundle.has_new() {
            return;
        }

        debug!("Processing new messages took {:?}", start.elapsed());

        let start = Instant::now();

        // Separate panes that need history replay from those that just need the latest.
        let needs_history: Vec<TileId> = self
            .state
            .panes_tree
            .tiles
            .iter()
        ...
        let latest: Vec<TimedMessage> = self.message_bundle.iter_latest().cloned().collect();

        for (tile_id, tile) in self.state.panes_tree.tiles.iter_mut() {
            let Tile::Pane(pane) = tile else { continue };

            if needs_history.contains(tile_id) {
                for msg in self.message_broker.get_history() {
                    pane.update(Some(msg));
                }
            } else {
                for msg in &latest {
                    pane.update(Some(msg));
        ...
        debug!("Pane message processing took {:?}", start.elapsed());
    ...
    fn process_outgoing_messages(&mut self) {
        let commands: Vec<_> = self
            ...
            .filter_map(|(_, tile)| {
                if let Tile::Pane(pane) = tile {
                    Some(pane.drain_outgoing_commands())
                } else {
                    None
                }
            })
            .flatten()
            .collect();
        self.message_broker.process_outgoing_messages(commands);
    }
}
...
pub struct AppConfig {
    ...
    pub tlm_def_path: Option<PathBuf>,
    /// Path to the COSMOS command definition file.
    pub cmd_def_path: Option<PathBuf>,
}
